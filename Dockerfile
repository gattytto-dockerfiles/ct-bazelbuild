...
RUN usermod -u 1000 -l ${U_HOME} build -d /storage/${U_HOME} -s /bin/bash && \
    ...
    dnf install -y wget gcc-c++ python3.10 python3.10-devel.x86_64 awscli awscli gcc file which unzip findutils nodejs git patch dnf-plugins-core java-11-openjdk.x86_64 && \
